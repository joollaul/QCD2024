...
main111 mymain11 main285 mymain285: $(PYTHIA) $$@.cc
ifeq ($(RIVET_USE),true)
...
# MPICH, HDF5, HIGHFIVE, and HepMC3.
mymain46 mymain286: $(PYTHIA) $$@.cc
ifeq ($(HDF5_USE)$(HIGHFIVE_USE)$(HEPMC3_USE)$(RIVET_USE),truetruetruetrue)
	$(MPICH_BIN)mpic++ $@.cc -o $@ -w $(CXX_COMMON:c++11=c++14) $(MPICH_INCLUDE)\
	 $(HDF5_INCLUDE) $(HIGHFIVE_INCLUDE) $(HEPMC3_INCLUDE)\
	 $(MPICH_LIB) $(HDF5_LIB) $(HEPMC3_LIB) -DHEPMC3 -DRIVET\
	  $(shell $(RIVET_BIN)$(RIVET_CONFIG) --cppflags --libs) $(RIVET_RPATH)
else
	$(error Error: $@ requires MPICH, HDF5, HIGHFIVE, RIVET and HEPMC3)
endif
